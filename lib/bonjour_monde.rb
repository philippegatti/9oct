...
def follow_hashtag (condition, number, client)
	...
	until followed_users == number
		...
		a+=1
		list = list.uniq
		followed_users = list.size
	end
	puts list
	for i in 0..number-1
		client.follow(list[i])
	end
end

def perform3()
	client=login()
	follow_hashtag("#bonjour_monde",5,client)
end
...
def stream(condition,client,client2)
	
	client.filter(track: condition) do |object|
  		puts object.text
...
def perform4(condition)
	client=login_streaming
	client2=login
	stream(condition,client,client2)
end

perform4("#bonjour_monde")